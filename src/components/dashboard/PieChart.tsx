...
export function Chart() {
  ...
  if(isLoading) {
    return <div className="h-[280px] flex items-center justify-center"><Loader /></div>;
  }

  if(isError) {
    return <div className="text-muted-foreground h-[280px] flex items-center justify-center text-sm">Cannot load data</div>;
  }

  return (
    <>
      <div className="w-full max-w-[280px] aspect-square flex items-center justify-center">
          <PieChart width={280} height={280} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
            {hasData && (
              <Pie
                data={dataToDisplay}
                dataKey="value"
            ...
            {hasData && (
              <Tooltip
                formatter={(_value, name) => {
                  ...
                  return [`$${item?.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, name];
                }}
                contentStyle={{
        ...
        {hasData && (
          <div className="flex flex-col w-full space-y-3 pt-2">
            {dataToDisplay.map((token) => (
              <div key={token.name} className="flex items-center gap-3 py-1.5 px-2 rounded-md hover:bg-muted/50 transition-colors">
                <img src={token.icon} alt={token.name} className="size-5 flex-shrink-0" />
                <span className="flex-1 text-sm font-medium">{token.name}</span>
                <span className="text-sm font-semibold text-muted-foreground">{((token.value / totalValue) * 100).toFixed(1)}%</span>
              </div>
            ))}
          </div>
        )}
